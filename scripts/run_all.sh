...
detect_hardware() {
    ...
    if [ -f /proc/meminfo ]; then
        ...
        # 使用bc计算浮点数GB，保留一位小数（如果bc可用）
        if command -v bc >/dev/null 2>&1; then
            TOTAL_MEM_GB=$(echo "scale=1; $TOTAL_MEM / 1024 / 1024" | bc)
        else
            # 使用awk进行浮点计算，或使用整数除法但添加小数估计
            TOTAL_MEM_GB=$(echo "$TOTAL_MEM 1024 1024" | awk '{printf "%.1f", $1/$2/$3}')
        fi
        echo "  系统内存: ${TOTAL_MEM_GB}GB"
    fi
    
    # GPU检测（不导出实验参数以避免环境变量污染，仅设置GPU_OPTIMIZED标志）
...
check_disk_space() {
    ...
    # 检查当前目录空间（使用--output参数避免列解析问题）
    if command -v df >/dev/null 2>&1; then
        # 使用--output=avail获取可用空间列，兼容现代df版本
        if df --help 2>&1 | grep -q -- "--output"; then
            AVAILABLE=$(df -m --output=avail . 2>/dev/null | tail -1)
        else
            # 回退到传统方法，但添加更多错误处理
            AVAILABLE=$(df -m . 2>/dev/null | awk 'NR==2 {print $4}' || echo "")
        fi
        # 验证AVAILABLE是否为数字
        if [ ! -z "$AVAILABLE" ] && [[ "$AVAILABLE" =~ ^[0-9]+$ ]]; then
            ...
            echo "  警告: 无法获取有效的磁盘空间信息（获取到: '${AVAILABLE}'）"
            return 0
        fi
    else
...
run_script() {
    ...
    if [ -f "$script_name" ]; then
        ...
        if [[ "$script_name" == *.py ]]; then
            ...
            # 捕获管道退出码（兼容所有Bash环境）
            # 如果PIPESTATUS可用则使用，否则使用备选方法
            if [ -n "${PIPESTATUS[0]}" ] 2>/dev/null; then
                exit_code=${PIPESTATUS[0]}
            else
                # 备选方法：从进程替换获取
                exit_code=$?
                # 检查是否为管道错误，如果是则尝试更复杂的方法
                if [ $exit_code -eq 0 ] || [ $exit_code -gt 128 ]; then
                    # 使用临时文件方法
                    local tmp_exitcode=$(mktemp)
                    if [ "$GPU_OPTIMIZED" -eq 1 ]; then
                        $PYTHON_CMD "$script_name" --gpu_optimized > >(tee "$log_file") 2>&1; echo $? > "$tmp_exitcode"
                    else
                        $PYTHON_CMD "$script_name" > >(tee "$log_file") 2>&1; echo $? > "$tmp_exitcode"
                    fi
                    sleep 0.1
                    exit_code=$(cat "$tmp_exitcode" 2>/dev/null || echo 1)
                    rm -f "$tmp_exitcode"
                fi
            fi
        else
...
if command -v df >/dev/null 2>&1; then
    ...
    if [ ! -z "$AVAILABLE" ] && [[ "$AVAILABLE" =~ ^[0-9]+$ ]]; then
        ...
        echo "无法获取有效的磁盘空间信息（获取到: '${AVAILABLE}'）"
    fi
fi
